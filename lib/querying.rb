...
def select_character_names_and_number_of_books_they_are_in
  "SELECT name, COUNT(books) FROM characters INNER JOIN character_books ON characters.id = character_books.character_id INNER JOIN books on books.id = character_books.book_id  GROUP BY characters.name"
end
